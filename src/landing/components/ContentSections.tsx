...
export function ConversionSection({ chatHref, language, onEnter }: Pick<SectionProps, 'chatHref' | 'language' | 'onEnter'>) {
  const copy = localizedCopy[language];
  return <RevealSection className="conversion-section section-shell" id="contact" onEnter={onEnter}><div className="conversion-copy"><p className="section-kicker"><MarketingText>{copy.conversionKicker}</MarketingText></p><h2><MarketingText>{copy.conversionTitle}</MarketingText></h2><p><MarketingText>{copy.conversionDescription}</MarketingText></p><a className="conversion-chat" href={chatHref} rel="noreferrer" target="_blank"><MessageCircle size={20} />{copy.chat}<ArrowRight size={18} /></a><small>{copy.formNotice}</small></div><DemoRequestForm language={language} /></RevealSection>;
}
